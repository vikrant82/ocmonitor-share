...
class SQLiteProcessor:
    ...
    def find_database_path(custom_path: Optional[Path] = None) -> Optional[Path]:
        ...
        import os
        if os.name == 'nt':
            windows_path = Path(os.environ.get('APPDATA', '')) / "opencode" / "opencode.db"
    ...
    @classmethod
    def parse_message_data(cls, message_data_str: str, session_id: str) -> Optional[InteractionFile]:
    ...
    @classmethod
    def load_session_messages(cls, conn: sqlite3.Connection, session_id: str) -> List[InteractionFile]:
    ...
    @classmethod
    def load_session_data(cls, conn: sqlite3.Connection, session_row: sqlite3.Row) -> Optional[SessionData]:
    ...
    def load_session_hierarchy(
        ...
        root_sessions.sort(
            key=lambda x: x['session'].start_time or datetime.min,
            reverse=True
        )
    ...
    @classmethod
    def get_most_recent_workflow(cls, db_path: Optional[Path] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            """, (main_session.session_id,)).fetchall()
